...
- **N-hop DESCRIBE parallel fetch (2026-05-28)**: replaced sequential per-subject `for subj in &next { get_entity_quads_cold(subj).await }` with `futures::future::try_join_all` — each hop's subject fetches run concurrently. Critical for distributed BlockStore (DistributedBlockStore peer fetches, KuboBlockStore HTTP RTT) where serial latency dominates. Sequential Phase 13 baseline: tree(4^6) 6-hop = 25.6s for 10K quads; parallel version expected ≈10-100× speedup proportional to per-hop frontier size (largest hop in tree has 4K nodes). All 6 nhop tests still green; **195 kotoba-graph tests pass**
