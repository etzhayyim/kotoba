...
- **Batched bulk ingest endpoint (2026-05-28)**: `POST /xrpc/ai.gftd.apps.yata.kg.ingest_batch` accepts `{ entities: [KgIngestReq] }` up to 1000 per request. All-or-nothing validation (entire batch rejected if any entity fails). Live measurement (release, KOTOBA_IPFS=off): **1000-entity batch in 0.25s = 3981 entities/sec — 142× speedup vs single-ingest** (28/sec). 4000 total quads written. Subsequent SELECT on 1000-entity graph (334 admins): seq p50 **2.49ms / 343 QPS**; c=16 p50 6.63ms / **2006 QPS**. 269 kotoba-server unit tests still pass.
...
- **CACAO-gated sustained HTTP bench (2026-05-28)**: `kotoba bench --cacao-seed <hex> --cacao-graph <scope> --cacao-private` — each request signs a fresh CACAO with a unique `(worker_id, iter)` nonce, sidestepping the CAIP-74 single-use replay guard.  Server booted with `KOTOBA_DEFAULT_VISIBILITY=private` + matching seed, 500 ingested entities (250 admins).  Same `SELECT * WHERE { ?s <kg/claim/role> "admin" }` returning 250 quads:
  - **CACAO-gated** seq: p50 **2.06 ms / 365 QPS**, 200/200 successful
  - **CACAO-gated c=16**: p50 12.12 ms / **950 QPS**, 478/500 successful (some nonce-store contention under heavy concurrency)
  - Unauthed baseline at similar scale: ~2-3K QPS — **CACAO auth overhead ≈ 3-5×**, dominated by Ed25519-verify per request on the server.  Acceptable for production CACAO ecosystems where the auth path is the trust boundary, not the hot path.
