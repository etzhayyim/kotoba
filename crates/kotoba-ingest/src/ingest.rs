...
use kotoba_crypto::envelope::encrypt_field;
...
impl EmailIngestor {
    pub fn new(
        ...
        Self { secure_vault, quad_store, vault_key, graph_cid, owner_did }
    }
    ...
    pub async fn ingest_raw(&self, raw: &[u8], thread_id: &str) -> Result<KotobaCid> {
        ...
        // ── 1. body → SecureVault (AES-256-GCM ciphertext blob) ─────────────
        let body_text = msg.body_text(0)
            .map(|c| c.into_owned())
            .unwrap_or_default();
        let blob_ref = self.secure_vault
            .put(&self.vault_key, Bytes::from(body_text.into_bytes()))
            .await
            .context("SecureVault::put body failed")?;
        ...
        let enc_from    = encrypt_field(&self.vault_key, from_str.as_bytes())
            .context("encrypt from")?;
        let enc_to      = encrypt_field(&self.vault_key, to_str.as_bytes())
            .context("encrypt to")?;
        let enc_subject = encrypt_field(&self.vault_key, subject_str.as_bytes())
...
mod tests {
    ...
    async fn body_stored_as_ciphertext_not_plaintext() {
        ...
        // Verify body_cid quad exists and refers to encrypted data
        let graph_cid = graph_cid_for("did:plc:test2");
